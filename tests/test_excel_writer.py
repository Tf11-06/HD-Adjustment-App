...
def test_header_row2_has_invoice_cols(xlsx_path):
    ...
    assert "Adjustment #" not in [ws.cell(2, ci).value for ci in range(1, len(HEADER_COLS) + 1)]


def test_header_row2_has_li1_credit_cols(xlsx_path):
    w = ExcelWriter(xlsx_path)
...
def test_header_row2_has_debit_li_cols(xlsx_path):
    w = ExcelWriter(xlsx_path)
    w.initialize_headers(2)
    wb = load_workbook(xlsx_path)
    ws = wb.active
    li2_start = len(HEADER_COLS) + len(LI1_COLS) + 1
    for ci, name in enumerate(LI_COLS, start=0):
        assert ws.cell(2, li2_start + ci).value == name
...
def test_append_invoice_header_fields_correct(xlsx_path):
    ...
    assert ws.cell(3, 3).value == "2026-02-24"   # adj_date
    assert ws.cell(3, 6).value == "C - Credit"   # credit_debit
    assert ws.cell(3, 7).value == pytest.approx(278.03)  # amount


def test_rule1_li1_only_has_credit_summary_fields(xlsx_path):
    w = ExcelWriter(xlsx_path)
    w.initialize_headers(2)
    w.append_invoice(_INVOICE_1)
    wb = load_workbook(xlsx_path)
    ws = wb.active
    li1_start = len(HEADER_COLS) + 1
    assert ws.cell(3, li1_start).value == "24"
    assert ws.cell(3, li1_start + 1).value == "7573"
    assert ws.cell(3, li1_start + 2).value == "C - Credit"
    assert ws.cell(3, li1_start + 3).value == pytest.approx(278.03)
    assert ws.cell(2, li1_start + len(LI1_COLS)).value == "SKU"
...
def test_rule3_compact_format_li1_all_blank(xlsx_path):
    ...
    ws = wb.active
    li1_start = len(HEADER_COLS) + 1
    for offset in range(len(LI1_COLS)):
...
def test_expand_adds_new_li_column_group(xlsx_path):
    ...
    wb = load_workbook(xlsx_path)
    ws = wb.active
    li4_start = len(HEADER_COLS) + len(LI1_COLS) + 1 + 2 * len(LI_COLS)
...
def test_expand_does_not_shrink(xlsx_path):
    ...
    wb = load_workbook(xlsx_path)
    ws = wb.active
    li6_start = len(HEADER_COLS) + len(LI1_COLS) + 1 + 4 * len(LI_COLS)
